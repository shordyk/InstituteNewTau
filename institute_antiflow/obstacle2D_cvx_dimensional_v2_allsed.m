...
% ch_decay = 7E3;
% ch_loc = 0E3;
% Numerical experiment #3 (all plastic, constant)
% sed_const_l = 36E3;
% sed_const_r = 18.15E3;
% sed_var = 12E3;
% sed_trans_l = -23E3;
...
% ch_decay = 20E3;
% ch_loc = 17E3;
% Numerical experiment #3 (all plastic, linear)
% sed_const_l = 27.1E3;
% sed_const_r = 15.2E3;
% sed_var_l = 27E3;
% sed_var_r = 20E3;
...
tau_c =@(x,y,u) (...
                 ...
                 heaviside(x - sed_trans_l).*heaviside(ch_loc - x).*(sed_var_l*(x/80E3) + ch_str*(exp(-abs(x-ch_loc)/ch_decay)) + sed_const_l).*abs(u) + ...
                 heaviside(x - ch_loc).*(sed_var_r*((80E3-x)/80E3) + ...
...
plot(sed_trans_l:1E2:ch_loc,sed_var_l*((sed_trans_l:1E2:ch_loc)/80E3) + ch_str*(exp(-abs((sed_trans_l:1E2:ch_loc)-ch_loc)/ch_decay)) + sed_const_l,'LineWidth',3,'Color',[0.8500, 0.3250, 0.0980])
hold on
plot(ch_loc:1E2:sed_trans_r,sed_var_r*((80E3-(ch_loc:1E2:sed_trans_r))/80E3) + ch_str*(exp(-abs((ch_loc:1E2:sed_trans_r)-ch_loc)/ch_decay)) + sed_const_r,'r','LineWidth',3,'Color',[0.8500, 0.3250, 0.0980])
